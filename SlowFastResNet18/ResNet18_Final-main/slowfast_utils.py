...
class ResNetBasicHead(nn.Module):
    ...
    def __init__(
        ...
        if isinstance(num_classes, (list, tuple)):
            ...
            self.projection_noun = nn.Linear(sum(dim_in), num_classes[0], bias=True)
        else:
...
class ResStage(nn.Module):
    ...
    def __init__(
        ...
        assert all(
            (
                ...
                for i in range(len(temp_kernel_sizes)-1)
            )
        )
    ...
    def _construct(
        ...
        for pathway in range(self.num_pathways-1):
            for i in range(self.num_blocks[pathway]-1):
